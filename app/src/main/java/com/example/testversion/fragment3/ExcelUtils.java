...
public class ExcelUtils {
    ...
    public static <T> void writeDataToExcel(Queue<Queue<Float>> queues, String fileName, Context c) {
...
//              sheet.mergeCells()
                ...
                for (int j = 0; j < queues.size(); j++) {
                    Queue<Float> que = queues.poll();
                    for (int i = 0; i < que.size(); i++) {
                        DecimalFormat decimalFormat=new DecimalFormat(".000");//构造方法的字符格式这里如果小数不足2位,会以0补足.
                        String p=decimalFormat.format(que.poll());
                        sheet.addCell(new Label(i, j + 1,p,arial12format));
                        if (p.length() <= 5){
                            sheet.setColumnView(i,p.length()+8); //设置列宽
                        }else {
                            sheet.setColumnView(i,p.length()+5); //设置列宽
                        }
                    }
